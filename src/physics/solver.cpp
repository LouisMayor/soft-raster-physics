...
bool solver::create_proxy(
	u32 ParticleID,
	const FVector2 &Position,
	const FVector2 &Initialvelocity,
	f64 Mass,
	f64 DragCoefficient,
	f64 MediumDensity,
	f64 SurfaceArea,
...
std::optional<ProxyValues> solver::get_proxy_values(
	u32 InParticleID
) const {
...
void solver::advance_step(
	std::vector<std::unique_ptr<proxy>> &InProxies,
	const FVector2 &WorldBounds,
	f64 dt
) const {
	std::for_each(std::execution::par_unseq, std::begin(InProxies), std::end(InProxies), [&](const std::unique_ptr<proxy> &proxy_instance) {
		proxy_instance->get_velocity() += solve_forces(
			proxy_instance->get_velocity(), proxy_instance->get_mass(), proxy_instance->get_drag_coefficient(), proxy_instance->get_medium_density(),
			proxy_instance->get_surface_area(), dt
		);
...
bool solver::solve_world_collision(
	const FVector2 &NextPosition,
	const FVector2 &WorldBounds,
	OUT FVector2 &WorldCollisionLocation
) const {
	bool no_hit = true;
...
void solver::solve_particle_collision(
	const proxy *const ParticleA,
	const proxy *const ParticleB,
	const FVector2 &ImpactDirection,
	OUT FVector2 &OutParticleAVelocity,
	OUT FVector2 &OutParticleBVelocity
) const {
...
std::tuple<
	FVector2,
	FVector2>
solver::get_force_from_particle_collision(
	const proxy *const ParticleA,
	const proxy *const ParticleB
) const {
	FVector2 A, B;
